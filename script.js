// Simulated JSON data for employees
const employees = [
...
// Function to fetch employees (simulated async operation)
...
async function displaySortedEmployees() {
  try {
    const fetchedEmployees = await fetchEmployees(); // Fetch employees
    const sortedEmployees = fetchedEmployees.sort((a, b) => a.age - b.age); // Sort by age
    console.log(sortedEmployees); // Log sorted employees
  } catch (error) {
